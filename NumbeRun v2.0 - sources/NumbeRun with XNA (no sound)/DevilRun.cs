...
public class DevilRun
{
    ...
    public void vMuoviPersonaggio(double gameTimeSeconds, int iNumeroMostro, int iPlayerAvatarX, int iPlayerAvatarY, int iPlayerAvatarXOffset, int iPlayerAvatarYOffset)
    {
        ...
        // Controllo se le coordinate sulla X sono uguali a quelle del personaggio
        if ((VariabiliGlobali.Globals.myMaps.iMapX + VariabiliGlobali.Globals.myMainPersonaggio.iPlayerAvatarXOffset) == (iPlayerAvatarX + iPlayerAvatarXOffset))
        {
            iDirection = 2;
        }
        // Controllo se le coordinate sulla Y sono uguali a quelle del personaggio
        if ((VariabiliGlobali.Globals.myMaps.iMapY + VariabiliGlobali.Globals.myMainPersonaggio.iPlayerAvaterYOffset) == (iPlayerAvatarY + iPlayerAvatarYOffset))
        {
            iDirection = 1;
        }
