...
public class PhantRun
{
    ...
    {
        bool bMovimentoConsentito = true;
    ...
    {
        int iPosizioneDaControllare = VariabiliGlobali.Globals.myMaps.iMapTrans[sY, sX];
        // Imposta la velocità del mostro
        iVelocity = 0.65;
        ...
        if (iPosizioneDaControllare != 014)
        { 
            while (iPosizioneDaControllare != 014)
            {
                sX = rRandom.Next(1, VariabiliGlobali.Globals.myMaps.MapWidth);
                sY = rRandom.Next(1, VariabiliGlobali.Globals.myMaps.MapHeight);
                iPosizioneDaControllare = VariabiliGlobali.Globals.myMaps.iMapTrans[sY, sX];
            }
        }
        int iDirection = rRandom.Next(1, 3);
        int mov = 0;

        if (sX == (iPlayerAvatarX + iPlayerAvatarXOffset))
        {
            iDirection = 2;
        }

        if (sY == (iPlayerAvatarY + iPlayerAvatarYOffset))
        {
            iDirection = 1;
        }
